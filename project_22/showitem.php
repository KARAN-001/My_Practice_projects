...
$safe_item_id = mysqli_escape_string($con, $_GET['item_id']);
...
$get_item_res=mysqli_query($con,$get_item_sql) or die (mysqli_error($con));
...
if(mysqli_num_rows($get_item_res) < 1) {
 ...
 $get_colors_sql="SELECT item_color FROM store_item_color WHERE item_id = '".$safe_item_id."' ORDER BY item_color";
  ...
  $get_colors_res = mysqli_query($con, $get_colors_sql)
   ...
   $get_sizes_sql = "SELECT item_size FROM store_item_size WHERE
    ...
    $get_sizes_res = mysqli_query($con, $get_sizes_sql)
   or die(mysqli_error($con));
